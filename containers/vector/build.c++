...
{
    auto beautify = [](size_t s){
        std::string spaces("  ");
        for (size_t i = 0; i < s; ++i)
            spaces+="  ";
        return "\n"+spaces+"["+std::to_string(s)+"]->" ;};
    try
    {
        ...
        for (size_t i = 0; i < ShinyStringVector.size(); ++i) { std::cout << beautify(i) << ShinyStringVector[i]; }
        std::cout << '\n';
...
bool test_contents()
{
    ...
    try
    {
        ...
        for (size_t i = 0; i < ShinyVector.size(); ++i) { std::cout << beautify(i) << ShinyVector[i]; }
        std::cout << '\n';
